...
void load_relay_config(const char * config_scope, const char * config_filename) 
{
...
{
	int opt;
	while ((opt = getopt(argc, argv, "rf:h:p:l:")) != -1) {
		switch (opt) {
		case 'f':
			memset( g_config_filename, 0, sizeof( g_config_filename ));
			strncpy( g_config_filename, optarg, sizeof( g_config_filename )-1);
			break;
		case 'r':
			g_relay = true;
			break;
	...
	if( strcmp( g_config_filename,"" )!=0) {
		load_relay_config( "test_1", g_config_filename ) ;
	}
