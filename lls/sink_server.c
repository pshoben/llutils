...
static bool g_sink_mode = false;
static bool g_relay_mode = false;  // if true, relay mode else echo mode
...
void server_run()
{
	...
	if( g_relay_mode && g_relay_sock_fd !=0 ) {
...
{
	int opt;
	while ((opt = getopt(argc, argv, "rsh:p:l:")) != -1) {
		switch (opt) {
		case 'r':
			g_relay_mode = true;
			break;
		...
		case 'l':
			...
			break;
		default:
			printf("usage: %s [-r -s [-h relay-to-host] [-p relay-to-port] [-l listen-port]\n", argv[0]);
			exit(1);
		}
	}
	printf("server listening on port %hu : relay mode = %d sink mode = %d\n", g_listen_port, g_relay_mode, g_sink_mode );
	if( g_relay_mode ) {
		printf("relay to %s:%hu\n",g_relay_to_host, g_relay_to_port);
		connect_to_relay_target();
	}
	if( g_sink_mode ) {
		printf("sink mode : no echo\n");
	}
	server_run();
	return 0;
}
